...
from uusio.scheduler.deadline_checker import (
    WARNING_THRESHOLDS,
...
async def test_send_warnings_calls_email_for_each_user():
    ...
    deadline = _make_deadline(7)
    with patch(
        "uusio.scheduler.deadline_checker.send_deadline_warning",
...
def test_email_body_content():
    from uusio.notifications.email import _build_body
    body = _build_body(
...
def test_email_html_body_contains_subject():
    from uusio.notifications.email import _build_html_body
